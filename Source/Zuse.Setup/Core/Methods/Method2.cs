...
namespace Zuse.Setup.Core.Methods
{
    class Method2 : IMethod
    {
        ...
        public void Install()
        {
            ...
            AssemblyDefinition zuseAssembly = AssemblyFactory.GetAssembly("Zuse.dll");

            MethodDefinition startMethod = null;
            
            foreach (TypeDefinition type in zuseAssembly.MainModule.Types)
            {
                if (type.Name != "<Module>")
                {
                    foreach (MethodDefinition method in type.Methods)
                    {
                        ...
                        {
                            zuneShellAssembly.MainModule.Inject(type);
                            startMethod = method;
                        }
                    }
                }
            ...
            foreach (Resource res in zuseAssembly.MainModule.Resources)
            {
                zuneShellAssembly.MainModule.Resources.Add(res);
            }
            ...
            // no need to copy, as Zuse.dll is completely injected into ZuneShell.dll
            //File.Copy("Zuse.dll", @"C:\Program Files\Zune\Zuse.dll", true);

            foreach (AssemblyNameReference asmref in zuseAssembly.MainModule.AssemblyReferences)
            {
            ...
            if (this.MethodEvent != null) this.MethodMessage(this, string.Format("Switched current directory to: {0:s}", Environment.CurrentDirectory));

            string output_asm = Consts.ZuneAssemblyLocation;

            if (this.MethodEvent != null) this.MethodMessage(this, string.Format("Outputing new patched executable: {0:s}", output_asm));
